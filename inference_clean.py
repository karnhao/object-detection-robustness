...
from tool import start_prediction_2
...
dataset = NuImagesCOCODataset("data/sets/nuimages", gt_path)
output_path = "outputs/autoencoder"
...
def predictions(model_name):
    if not os.path.exists(os.path.join(output_path, f"{model_name}_results_clean.json")):
        ...
        start = time.time()
        start_prediction_2(model=model, dataset=dataset, showProgress=True, save_every_n=100, result_path=output_path)
        ...
        _save_model_stats(model_name, "clean", params_stats, timing_stats)

    else:
        print(f"{model_name} clean results already exists.")
