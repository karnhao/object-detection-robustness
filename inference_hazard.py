...
generated_hazards = os.listdir("./data/sets/generated/nuimages-1k")
device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
print(f"using {device}")

gt_path = "data/sets/nuimages/nuimages_1k.json"
result_path = "outputs/nuimages-1k"
...
def _ensure_outputs_dir():
    out_dir = os.path.join(".", result_path)
...
def hazard_predictions(model_name, hazard):
    # Debuging. Remove True after done
    if True or (not all(os.path.exists(f"./outputs/{model_name}_results_{hazard}_{i}.json") for i in range(1, 4))):
        ...
        run_predictions_for_hazard(
            ...
            hazard_type=hazard,
            severity_levels=[3],
            parallel=False, # Debuging. Replace Flase with: parallel_settings[model_name]
            saveLocation=None,
            ann_file=gt_path,
            batch_size=1,
            save_every_n=100,
